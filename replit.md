...
These principles are ratified and authoritative. They govern all feature work, AI assistance, and architectural decisions on this project.
...
- **Forecasting must always show confidence levels** — never present AI predictions as certainties. This governs future par level suggestions, order quantity forecasting, and theoretical yield predictions.
...
- **Gradual decomposition over monolith expansion** — `server/routes.ts` is currently >17,000 lines. It must not grow unnecessarily. New routes should be grouped by domain where practical: `inventory`, `recipes`, `vendors`, `purchasing`, `prep`, `forecasting`, `AI ingestion`, `auth/admin`.
- **Shared business logic into service functions** — logic should not live directly inside route handlers. Extract into service functions incrementally.
...
- **"Sweep scan"** and **"catch-weight scan"** are internal/product terms — not validated with users. Use plain language in UI copy:
  - "Scan shelves" (not "sweep scan")
  - "Review counted items" (not "apply scan results")
  - "Capture variable-weight items" (not "catch-weight scan")
  - "Confirm inventory" (not "apply session")
  - "Build order" (not "create PO")
- When a product-specific term is necessary, pair it with a short helper description.
- **AI suggestions must be visually distinct from confirmed data** — use visual treatment (badge, color, icon) to differentiate AI-proposed values from operator-entered ones.
...
- **Confidence scoring is required for**: OCR results, inventory item recognition, forecasting outputs, recipe extraction. Low-confidence outputs must be flagged for human review before use.
- **Low-confidence outputs go to a review queue** — never auto-apply below a defined confidence threshold.
