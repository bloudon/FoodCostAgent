...
- **Purchase Order Management**: Unit/case ordering, vendor-specific filtering, keyboard-optimized entry. Expected date is required for all new purchase orders with multi-layer validation (HTML5, frontend toast, backend schema).
- **Terminology Standardization**: Consistent use of "inventory item."
- **Receiving Module**: Supports partial receipts, resumable sessions, visual indicators, and correct PO pricing. Receiving page allows editing unit prices on unsaved rows (prices lock after save), enabling price adjustments when items are received at new pricing. Saved prices can be re-edited by clicking them. Uses nullish coalescing to preserve zero-price values. All totals (line totals, actual value) reflect edited prices in real-time. **Price Update Fix**: Backend correctly treats receipt_lines.price_each as unit price (per lb/oz/etc) and updates inventory_items.pricePerUnit directly without dividing by case size, fixing a critical bug that was causing dramatically reduced prices (e.g., $3.29 incorrectly becoming $0.07 when case size was 48).
...
- **Company Data Purge System (Development Only)**: Cascading delete functionality for purging all data associated with a company, proving multi-tenant isolation. Components: (1) CLI script (`server/scripts/purge-company.ts`) with dry-run mode and interactive confirmation; (2) Admin API endpoint (`DELETE /api/admin/companies/:id/purge`) restricted to development environment and global_admin role; (3) Frontend UI in Companies page with trash icon button (visible only in `import.meta.env.DEV`), AlertDialog confirmation, and toast notifications. Deletes in dependency order (leaf-to-root) across 40+ tables including POS sales, recipes, transfers, waste, inventory counts, purchase orders, vendor data, inventory items, menu items, stores, and company record. **User Safety:** Users are NEVER deleted during company purge - they may belong to multiple companies or have global admin privileges and should be managed separately. **Proven isolation:** Test verified purging Company A (8 rows across 7 tables) while Company B data remained completely untouched, demonstrating robust multi-tenant data separation. **Known Limitation:** Dry-run mode currently skips deletions but returns zero counts instead of actual row counts; future enhancement would add COUNT queries for accurate preview totals.
