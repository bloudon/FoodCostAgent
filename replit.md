...
## Overview
This project is a comprehensive inventory management and recipe costing system designed for food service businesses, especially pizza restaurants, operating in multi-company environments. Its primary goal is to optimize operations, reduce waste, and improve profitability. Key capabilities include advanced unit conversions, support for nested recipes, real-time POS sales integration, detailed variance reporting, dual pricing models (Last Cost and Weighted Average Cost), and vendor price comparison for purchase orders.
...
The system employs a multi-tenant architecture with data isolation enforced at company and store levels. It integrates with Thrive Control Center (TCC).

### Frontend
- **Framework**: React 18 with TypeScript and Vite.
- **UI**: `shadcn/ui` components (Radix UI, Tailwind CSS) with custom theming, dark/light modes, and responsive navigation.
...
- **Authentication & Sessions**: Session-based authentication with `selected_company_id`.
...
- **Receiving Module**: Supports partial receipts, resumable sessions, and allows editing unit prices with real-time updates.
- **Vendor Integration**: Pluggable adapter pattern for distributors (Sysco, GFS, US Foods).
...
- **Store-to-Store Transfer Orders**: Tracks inventory movement through a defined workflow (pending → Execute Transfer → in_transit → Receive Transfer → completed).
- **Waste Tracking Module**: Comprehensive waste logging with store-level isolation for inventory and menu items, touch-friendly UI, automatic value calculation, and various waste reasons.
- **HMAC Authentication for Inbound Data Feeds**: Implemented hierarchical HMAC-SHA256 bearer token authentication for securing API integrations (POS, vendor EDI), including company-level credentials, cryptographically secure keys, timestamp/nonce validation, content MD5, and optional IP whitelisting.
...
- **Database Connection Pooling**: Neon Pool configured for optimal performance with Replit Autoscale.
- **Composite Indexes**: 13 strategic indexes added for multi-tenant query patterns across critical tables.
- **Database Transactions**: `withTransaction()` utility applied to Inventory Count Application, Receipt Completion, Transfer Order Execution, and Transfer Order Receipt for atomic operations.
- **Tenant Isolation Pattern**: All transactional operations include explicit `companyId` predicates for multi-tenant security.
- **Session Management**: Background job cleans up expired `auth_sessions` every 60 minutes.
- **Storage Layer Security**: Critical storage methods require `companyId` for explicit tenant filtering.
