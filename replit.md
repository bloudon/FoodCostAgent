...
- **Stripe Integration (PENDING)**: Subscription billing planned using Stripe. Seed script at `scripts/stripe_seed_subscriptions.mjs` creates Basic/Pro products and monthly/quarterly/annual prices in test mode. Credentials managed manually (NOT via Replit OAuth integration — user declined). Required secrets: `STRIPE_SECRET_KEY` (sk_test_... for dev, sk_live_... for prod), `STRIPE_WEBHOOK_SECRET`. Run seed script with: `STRIPE_SECRET_KEY=sk_test_... node scripts/stripe_seed_subscriptions.mjs`