...
// ─────────────────────────────────────────────────────────────────────────────
// Section 9: DB integration — tenant isolation
// ─────────────────────────────────────────────────────────────────────────────

describe("DB integration — tenant isolation for routing", () => {
  // Import DB dependencies lazily in the describe block so tests that don't
  // need the DB are not affected by connection errors in other environments.
  const PREFIX = "inttest-m3b-";

  const COMPANY_A_ID = `${PREFIX}company-A`;
  const COMPANY_B_ID = `${PREFIX}company-B`;
  const STORE_A_ID   = `${PREFIX}store-A`;
  const VENDOR_A_ID  = `${PREFIX}vendor-A`;
  const PO_A_ID      = `${PREFIX}po-A`;

  async function insertTenantFixtures() {
    ...
    // Company A (the owner of the PO)
    await db.insert(companies).values({
      id: COMPANY_A_ID,
      name: "Tenant Isolation Test — Company A",
      country: "US",
    ...
    await db.insert(companies).values({
      id: COMPANY_B_ID,
      name: "Tenant Isolation Test — Company B",
      country: "US",
      timezone: "America/New_York",
      preferredUnitSystem: "imperial",
      costingMethod: "last_cost",
  ...
  async function deleteTenantFixtures() {
    const { db } = await import("../db");
    const { companies, companyStores, vendors, purchaseOrders } = await import("@shared/schema");
  ...
  afterEach(deleteTenantFixtures);

  it("company B cannot read company A's PO — getPurchaseOrder returns null (tenant isolation)", async () => {
    await insertTenantFixtures();

    const { db } = await import("../db");
    const { purchaseOrders } = await import("@shared/schema");
    const { eq, and } = await import("drizzle-orm");

    // Mirrors what storage.getPurchaseOrder(poId, companyId) does:
    // SELECT ... FROM purchase_orders WHERE id = ? AND company_id = ?
    const [poFromCompanyB] = await db
      .select({ id: purchaseOrders.id })
      .from(purchaseOrders)
      .where(
        and(
          eq(purchaseOrders.id, PO_A_ID),
          eq(purchaseOrders.companyId, COMPANY_B_ID) // wrong company
        )
      )
      .limit(1);

    // Tenant isolation: company B cannot see company A's PO
    expect(poFromCompanyB).toBeUndefined();
  });

  it("the PO exists for company A — cross-company access would trigger 403 not 404", async () => {
    await insertTenantFixtures();
    ...
    // Mirrors the cross-company 403 check in the route endpoint:
    // SELECT id FROM purchase_orders WHERE id = ? (no company filter)
    ...
    // PO exists — route would return 403 ("Access denied") not 404
    expect(anyPo).toBeDefined();
    expect(anyPo?.companyId).toBe(COMPANY_A_ID);
  });

  it("company A can read its own PO (sanity: correct company is not blocked)", async () => {
    await insertTenantFixtures();
    ...
    const [poFromCompanyA] = await db
      .select({ id: purchaseOrders.id, status: purchaseOrders.status })
      .from(purchaseOrders)
      .where(
        and(
          eq(purchaseOrders.id, PO_A_ID),
          eq(purchaseOrders.companyId, COMPANY_A_ID) // correct company
        )
      )
      .limit(1);

    expect(poFromCompanyA).toBeDefined();
    expect(poFromCompanyA?.id).toBe(PO_A_ID);
    expect(poFromCompanyA?.status).toBe("pending");
  });

  it("routing isolation: cross-company lookup returns null while same-company returns non-null", async () => {
    await insertTenantFixtures();
    ...
    // Company B's request — tenant-isolated
    const [poAsCompanyB] = await db
      .select({ id: purchaseOrders.id })
      .from(purchaseOrders)
      .where(and(eq(purchaseOrders.id, PO_A_ID), eq(purchaseOrders.companyId, COMPANY_B_ID)))
      .limit(1);

    // Company A's request — authorized
    const [poAsCompanyA] = await db
      .select({ id: purchaseOrders.id })
      .from(purchaseOrders)
      .where(and(eq(purchaseOrders.id, PO_A_ID), eq(purchaseOrders.companyId, COMPANY_A_ID)))
      .limit(1);

    // The 403 gate: poAsCompanyB is undefined (isolation) → route checks for the PO
    // in any company → finds poAsCompanyA → returns 403 not 404
    expect(poAsCompanyB).toBeUndefined();      // blocked by tenant filter
    expect(poAsCompanyA).toBeDefined();        // owner can access
    expect(poAsCompanyA?.id).toBe(PO_A_ID);
  });
});
