...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      if (topLevelForSeeding.length > 0) {
        try {
          ...
          console.error("[Onboarding Menu Scan Approve] Recipe seeding failed (non-fatal):", seedErr);
        }
      }
  ...
  app.post("/api/order-guides/process-pdf", requireAuth, async (req, res) => {
    try {
      ...
      const vendorProducts = products.map((p, index) => ({
        ...
        caseSizeRaw: p.caseSizeRaw ?? undefined,
        innerPack: p.innerPack ?? undefined,
      ...
      for (const product of vendorProducts) {
        ...
        lines.push({
          ...
          packSize: null,
          uom: product.unit || null,
          caseSize: (product as any).caseSize ?? null,
          caseSizeRaw: product.caseSizeRaw || null,
          innerPack: (product as any).innerPack ?? null,
          innerPackRaw: null,
          price: product.price ?? null,
  ...
  app.get("/api/inventory-items", requireAuth, async (req, res) => {
    ...
    const enriched: EnrichedInventoryItem[] = items.map((item) => {
      ...
      return {
        ...
        containerUnitId: item.containerUnitId,
  ...
  app.delete("/api/inventory-count-entries/:entryId", requireAuth, async (req, res) => {
    try {
      ...
      const line = await storage.getInventoryCountLine(entry.inventoryCountLineId);
      if (!line) {
        return res.status(404).json({ error: "Count line not found" });
      }
      const count = await storage.getInventoryCount(line.inventoryCountId);
      ...
      const updatedLine = await storage.deleteCountEntry(req.params.entryId);
      res.json(updatedLine);
    } catch (error: any) {
      res.status(400).json({ error: error.message });
    }
  ...
  //
  // POST /api/purchase-orders/:id/route-lines
  //
  // Moves selected lines from the source PO into vendor-specific draft POs.
  // Rules:
  //   - Caller specifies targetVendorItemId explicitly; server verifies same inventoryItemId
  //   - Whole-line routing only (no partial quantities)
  //   - All ineligibility reasons collected before rejecting (structured 422)
  //   - Idempotency: duplicate calls return prior audit result, not an error
  //   - Lines merged into existing draft POs when vendorItemId matches
  //   - Source PO lines deleted atomically within same transaction
  ...
  app.post("/api/purchase-orders/:id/route-lines", requireAuth, createRoutingPOGuard({
    ...
    try {
      ...
      const linesSchema = z.array(z.object({
        ...
        targetVendorItemId: z.string().min(1),
      })).min(1, "At least one line is required");

      const parseResult = linesSchema.safeParse(req.body.lines);
      if (!parseResult.success) {
        return res.status(400).json({ error: parseResult.error.errors[0]?.message ?? "Invalid lines" });
      }
      const lineRequests = parseResult.data;

      // ── 3. Load all data needed for validation ────────────────────────────
      const [sourceLines, allUnits] = await Promise.all([
        storage.getPOLines(sourcePoId),
        storage.getUnits(),
      ]);
      ...
      const sourceVendorItemRows = sourceViIds.length > 0
        ...
        : [];
      const sourceViMap = new Map(sourceVendorItemRows.map(vi => [vi.id, vi]));

      // Target vendor items — company-scoped via vendor join
      const targetViIds = [...new Set(lineRequests.map(r => r.targetVendorItemId))];
      const targetVendorItemRows = targetViIds.length > 0
        ? await db
          .select({ vi: vendorItems, vendorName: vendors.name })
          .from(vendorItems)
          .innerJoin(vendors, eq(vendorItems.vendorId, vendors.id))
          .where(
            and(
              inArray(vendorItems.id, targetViIds),
              eq(vendors.companyId, companyId),
              eq(vendors.active, 1),
            )
          )
        : [];
      const targetViMap = new Map(targetVendorItemRows.map(r => [r.vi.id, r]));

      // Inventory item units for incompatible-unit check
      const allInvItemIds = [...new Set(
        sourceVendorItemRows.map(vi => vi.inventoryItemId).filter((id): id is string => id != null)
      )];
      const invItemUnitRows = allInvItemIds.length > 0
        ? await db
          .select({ id: inventoryItems.id, unitId: inventoryItems.unitId })
          .from(inventoryItems)
      ...
      // Idempotency check — lines already successfully routed
      const existingAudits = lineRequests.length > 0
        ? await db
          .select()
          .from(poRoutingAudit)
          .where(
            and(
              inArray(poRoutingAudit.sourcePOLineId, lineRequests.map(r => r.poLineId)),
      ...
      type ValidLine = {
        ...
        targetViRow: typeof targetVendorItemRows[0];
        inventoryItemId: string;
        fromUnitPrice: number;
        fromCasePrice: number;
        toUnitPrice: number;
        toCasePrice: number;
        toCaseSize: number;
        toUnitId: string;
      ...
      const alreadyRouted: { poLineId: string; targetVendorItemId: string; destinationPoId: string; auditId: string }[] = [];
      const ineligibleLines: { poLineId: string; targetVendorItemId: string; reason: string }[] = [];
      ...
      for (const lr of lineRequests) {
        ...
        if (priorAudit) {
          alreadyRouted.push({
            ...
            targetVendorItemId: lr.targetVendorItemId,
            destinationPoId: priorAudit.destinationPoId,
            auditId: priorAudit.id,
          });
          continue;
        }

        // Line must be in source PO
        const line = sourceLinesById.get(lr.poLineId);
        if (!line) {
          ineligibleLines.push({ poLineId: lr.poLineId, targetVendorItemId: lr.targetVendorItemId, reason: "Line not found in source PO" });
          continue;
        }

        // Target vendor item must belong to this company
        const targetViRow = targetViMap.get(lr.targetVendorItemId);
        if (!targetViRow) {
          ineligibleLines.push({ poLineId: lr.poLineId, targetVendorItemId: lr.targetVendorItemId, reason: "Target vendor item not found or does not belong to your company" });
          continue;
        }
      ...
      // ── 5. Reject if any lines are ineligible (structured 422) ────────────
      if (ineligibleLines.length > 0) {
        return res.status(422).json({
          error: "One or more lines cannot be routed",
          ineligibleLines,
        });
      }
      ...
      console.error("[PO Route Lines]", error);
      return res.status(500).json({ error: error.message || "Routing failed" });
    }
  });

  // ── GET routing audit history for a source PO ────────────────────────────────
  app.get("/api/purchase-orders/:id/routing-audits", requireAuth, async (req, res) => {
    try {
      ...
      const audits = await db
        ...
        .where(
          and(
            eq(poRoutingAudit.sourcePoId, req.params.id),
            eq(poRoutingAudit.companyId, companyId)
          )
        )
  ...
  app.patch("/api/receipts/:id/complete", requireAuth, async (req, res) => {
    try {
      ...
      if (receipt.status === "completed") {
        return res.status(200).json({ message: "Receipt already completed" });
      }
...
Return format: ["ingredient1", "ingredient2", ...]`;
  ...
  /**
   * POST /api/menu-items/seed-recipes
   * On-demand seeding: for every menu item in the company that has a
   * non-empty `description` and no linked recipe, parse the description
   * into ingredient tokens and create a recipe stub + placeholder components.
   */
  app.post("/api/menu-items/seed-recipes", requireAuth, async (req, res) => {
    try {
      const companyId = req.companyId!;
      // Fetch all menu items for the company that have a description but no recipeId
      const candidateItems = await db
        .select({ id: menuItems.id, name: menuItems.name, description: menuItems.description, price: menuItems.price })
        .from(menuItems)
        .where(
          and(
            eq(menuItems.companyId, companyId),
            isNotNull(menuItems.description),
            isNull(menuItems.recipeId)
          )
        );

      const withDescriptions = candidateItems.filter(
        (item) => item.description && item.description.trim().length > 0
      ) as Array<{ id: string; name: string; description: string; price: number | null }>;

      if (withDescriptions.length === 0) {
        return res.json({ recipesCreated: 0, message: "No eligible menu items found — all items either already have a recipe or have no description." });
      }

      const { seedRecipesFromDescriptions } = await import('./services/menuRecipeSeeder');
      const seeded = await seedRecipesFromDescriptions(withDescriptions, companyId);

      return res.json({ recipesCreated: seeded.length });
    } catch (error: any) {
      console.error("[Seed Recipes from Menu Error]", error);
      res.status(500).json({ error: error.message });
    }
  });
  ...
  // Chat-log and chat-correction routes — see server/routes/chatLogsRoutes.ts
  ...
  app.post("/api/tfc/sales/upload", requireAuth, requireTier("platform"), upload.single('file'), async (req, res) => {
    try {
      ...
      for (const [key, rows] of salesByDateStore) {
        ...
        // Parse YYYY-MM-DD string to Date at midnight UTC to avoid timezone shifts
        const [year, month, day] = dateStr.split('-').map(Number);
        const salesDate = new Date(Date.UTC(year, month - 1, day));
...
- Only include [SUGGEST_HUMAN_HANDOFF] when you genuinely cannot resolve the issue yourself.${correctionsBlock}`;
   ...
   * Uses next() to pass through to /sessions/active when id === "active" so
   * the static route below handles that case correctly.
   */
  app.get("/api/mobile/sessions/:id", requireAuth, async (req, res, next) => {
    // Pass through to the /sessions/active handler (registered below)
  ...
  // These endpoints are only registered in non-production environments.
  // They exist solely to support automated integration tests.
  if (process.env.NODE_ENV !== 'production') {
    /**
     * POST /api/dev/test/menu-import-session
     * Creates a pending menuImportSessions record and returns its ID so that
     * integration tests can exercise the approve endpoint without a real AI scan.
     *
     * Body (all optional):
     *   storeId  — store to associate with the session
     */
    app.post("/api/dev/test/menu-import-session", requireAuth, async (req, res) => {
      try {
        const companyId = (req as any).companyId;
        if (!companyId) return res.status(400).json({ error: "Company context required" });
        // Resolve storeId: prefer explicit body param, then first company store
        let storeId: string | null = req.body?.storeId ?? null;
        if (!storeId) {
          const stores = await db.select().from(companyStores)
        ...
        console.error("[dev/test/menu-import-session]", err);
        return res.status(500).json({ error: err.message });
      }
    });
