/**
 * Cross-company isolation tests for POST /api/mobile/sweep-scan.
 *
 * Verifies that a mobile user from Company A cannot submit a sweep scan
 ...
 *
 * Endpoints covered:
 *   POST /api/mobile/sweep-scan  (storeId in multipart form body)
 *
...
test.describe('POST /api/mobile/sweep-scan — unauthenticated request rejected', () => {
...
test.describe('POST /api/mobile/sweep-scan — cross-company store isolation', () => {
  let tokenA: string;
  let tokenB: string;
  ...
  test(
    ...
    async ({ request }) => {
      ...
      expect(
        res.status(),
        'Expected 403 when Company A token submits a scan attributed to a Company B store',
      ).toBe(403);
    },
  );
  ...
  test(
    ...
    async ({ request }) => {
      ...
      expect(
        res.status(),
        'Expected 403 when Company B token submits a scan attributed to a Company A store',
      ).toBe(403);
    },
  );
  ...
  test(
    ...
    async ({ request }) => {
      ...
      // 200 (scan succeeded), 422 (no items found), or 500 (AI/image error) are all
      // acceptable — they prove the store ownership check passed.
